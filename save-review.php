...
function redirect_back($anchor = '#') {
  ...
  $url = parse_url($ref); $host = $_SERVER['HTTP_HOST'] ?? '';
  if (!isset($url['host']) || $url['host'] !== $host) { $ref = '/'; }
  if ($anchor && strpos($ref, $anchor) === false) { $ref .= $anchor; }
...
if (!$fh) { respond_json(['ok'=>false,'error'=>'storage_unavailable'], 500); }
flock($fh, LOCK_EX);
